...
\begin{document}
	\begin{tikzpicture}		
		...
		% Central-upper text
		\node at (16.8,10.9) [fascist] {\fontsize{60}{60}\selectfont FASCIST};
		
		% Dotted pattern for policies
		\fill [pattern=crosshatch dots, pattern color=fascist] (2.6,2.6) rectangle(6.5,9.4);
		\fill [pattern=crosshatch dots, pattern color=fascist] (7.5,2.6) rectangle(11.4,9.4);
		...
		\begin{scope}
			\fill [fascist, rounded corners] (13.35,8) rectangle (15.35,8.2);
			\fill [fascist, rounded corners] (13.35,7.7) rectangle (15.35,7.9);
			\fill [fascist, rounded corners] (13.35,7.4) rectangle (15.35,7.6);
			\fill [fascist, rounded corners] (13.35,5.3) rectangle (15.35,7.3);
		\end{scope}		
		...
		\node at (24.15,3.6) [white] {\fontsize{10}{10}\selectfont \begin{tabular}{c}
				THE PRESIDENT MUST\\ KILL A PLAYER.\\\\VETO POWER IS\\UNLOCKED.
		\end{tabular}};
		...
		\node at (24.25, 9.05) [fascist] {\fontsize{10}{10}\selectfont FASCISTS WIN IF HITLER IS ELECTED CHANCELLOR};
		...
		\fill [white] (7, 0.6) rectangle (26.6,1.5);
		\foreach \x in {2.3, 3.7, ...,6}
		{
		...
		\foreach \x in {31.8, 30.4, ..., 28}
		{
			\draw [white, line width=5, rounded corners=7] (\x,1.35) rectangle (\x + 0.9,0.75);
			\draw [white, line width=5] (\x - 0.8,1.05)--(\x + 0.3,1.05);
		}
		\draw [white, line width=5, rounded corners=7] (27.6,1.35) rectangle (27.6 + 0.9,0.75);
		
		% Text on lower side of the board
		\node at (16.8, 1) [fascist] {\fontsize{13}{13}\selectfont 5 OR 6 PLAYERS: PLAY WITH 1 FASCIST AND HITLER, HITLER KNOWS WHO THE FASCIST IS.};
